...
TEST_CASE("Modifying task relation") {
  ...
  const std::string user_input8 = "mod T10 -x";
  const auto [flag8, buffr8, sess8] = interpreter.parse(user_input8);
  const std::string relations_result8 =
      INTERPRETER_MOD::get_file_content(PATH_RELATIONS);
  const std::string relations_target8 =
      "0 2 1\n"
      "1 3 1\n"
      "2 5 2\n"
      "4 7 3\n"
      ...
      "10 1 2\n"
      "11 6 1\n";
  CHECK(flag8 == "Modified task relation");
  CHECK(relations_result8 == relations_target8);
}
